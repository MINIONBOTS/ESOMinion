...
function eso_task_grind:Init()
	...
	local ke_lootWindow = ml_element:create( "LootWindow", c_lootwindow, e_lootwindow, 280 )
	...
	local ke_nextMarker = ml_element:create( "NextMarker", c_nextgrindmarker, e_nextgrindmarker, 70 )
...
function c_findgrindable:evaluate()	
	...
	if (ValidTable(target)) then
		if (not target.attackable or target.dead or IsBlacklisted(target)) then
			needsUpdate = true
		end
	else
		needsUpdate = true
	...
	if (needsUpdate) then
		...
		if (ValidTable(newTarget)) then
			c_findgrindable.targetid = newTarget.id
			return true
		end
	end	
...
function c_nextgrindobjective:evaluate()
	...
	if (gatherid > 0 and gGather == "1") then
		gatherable = EntityList:Get(gatherid)
		if (ValidTable(gatherable)) then
			gatherdistance = gatherable.pathdistance
		end
	end
	...
	if (targetid > 0) then
		...
		if (ValidTable(target)) then
			targetdistance = target.pathdistance
		end
	end
