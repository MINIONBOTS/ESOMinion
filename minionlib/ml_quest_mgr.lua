ml_quest_mgr = {}
ml_quest_mgr.ModuleName = "GW2Minion" -- gets reset on init
ml_quest_mgr.profilepath = GetStartupPath() -- gets reset on init
ml_quest_mgr.RegisteredButtonEventList = {} -- holds the already registered events of the questmanager, needed since we cant unregister events 
...
function ml_quest_mgr.AddNewStep( step )
	...
	local mid =  e("GetCurrentMapIndex()")
	if ( not ml_quest_mgr.QuestIsForMapID( ml_quest_mgr.QuestList[ml_quest_mgr.editwindow.currentPrio],mid)) then
		ml_quest_mgr.QuestList[ml_quest_mgr.editwindow.currentPrio].mapid = ml_quest_mgr.QuestList[ml_quest_mgr.editwindow.currentPrio].mapid .. ","..tostring(e("GetCurrentMapIndex()"))
		ml_quest_mgr.QuestList[ml_quest_mgr.editwindow.currentPrio].map = ml_quest_mgr.QuestList[ml_quest_mgr.editwindow.currentPrio].map .. ","..e("GetMapName()")
	end
